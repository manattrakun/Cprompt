...
addSafeClick('.char-tab-btn:not(.config-tab-btn)',(btn)=>switchCharTab(btn.dataset.target||btn.getAttribute('data-target')));addSafeClick('.char-card:not(.mascot-card):not(.mascot-bg):not(.mascot-outfit):not(.mascot-expression)',(card)=>selectCharacter(card,card.dataset.value));addSafeClick('.config-header',(header)=>toggleConfig(header.dataset.target));addSafeClick('.config-option',(opt)=>selectConfigOption(opt));addSafeClick('.config-tab-btn',(btn)=>switchConfigTab(btn));addSafeClick('.segment-opt',(seg)=>selectSegment(seg))}
...
const btnSettings=document.getElementById('btn-settings');const settingsModal=document.getElementById('settings-modal');const closeSettingsBtn=document.getElementById('close-settings');const cancelSettingsBtn=document.getElementById('btn-cancel-settings');const saveSettingsBtn=document.getElementById('btn-save-settings');const geminiApiKeyInput=document.getElementById('gemini-api-key');const quotaModal=document.getElementById('quota-modal');const closeQuotaModalBtn=document.getElementById('close-quota-modal');const btnCloseQuotaModal=document.getElementById('btn-close-quota-modal');const veo3AspectRatioSelect=document.getElementById('veo3-aspect-ratio');function setupSettingsModal(){btnSettings.addEventListener('click',openSettingsModal);closeSettingsBtn.addEventListener('click',closeSettingsModal);cancelSettingsBtn.addEventListener('click',closeSettingsModal);saveSettingsBtn.addEventListener('click',saveSettings);settingsModal.addEventListener('click',(e)=>{if(e.target===settingsModal){closeSettingsModal()}})}
...
function setupSettingsModal(){btnSettings.addEventListener('click',openSettingsModal);closeSettingsBtn.addEventListener('click',closeSettingsModal);cancelSettingsBtn.addEventListener('click',closeSettingsModal);saveSettingsBtn.addEventListener('click',saveSettings);settingsModal.addEventListener('click',(e)=>{if(e.target===settingsModal){closeSettingsModal()}})}
...
function saveSettings(){const veo3AspectRatioSelect=document.getElementById('veo3-aspect-ratio');if(veo3AspectRatioSelect){const aspectRatio=veo3AspectRatioSelect.value;localStorage.setItem('veo3_aspect_ratio',aspectRatio)}
closeSettingsModal();showToast('บันทึกการตั้งค่าสำเร็จ!','success')}
function getGeminiApiKey(){return localStorage.getItem('gemini_api_key')||''}
...
function bananaSetupEventListeners(){const mascotSmartCheck=document.getElementById('mascot-smart-auto-checkbox');const mascotBgInput=document.getElementById('mascot-bg-select');const mascotOutfitInput=document.getElementById('mascot-outfit-select');const mascotCustomBg=document.getElementById('mascot-custom-bg-input');const mascotCustomOutfit=document.getElementById('mascot-custom-outfit-input');const mascotTextCheck=document.getElementById('mascot-text-overlay-checkbox');const mascotTextBox=document.getElementById('mascot-text-box');if(mascotSmartCheck){mascotSmartCheck.addEventListener('change',(e)=>{const isSmart=e.target.checked;const toggleLock=(el,lock)=>{if(!el)return;const group=el.closest('.input-group');if(group){if(lock)group.classList.add('disabled-section');else group.classList.remove('disabled-section')}};if(isSmart){toggleLock(mascotBgInput,!0);toggleLock(mascotOutfitInput,!0);if(mascotCustomBg)mascotCustomBg.disabled=!0;if(mascotCustomOutfit)mascotCustomOutfit.disabled=!0}else{toggleLock(mascotBgInput,!1);toggleLock(mascotOutfitInput,!1);if(mascotCustomBg)mascotCustomBg.disabled=!1;if(mascotCustomOutfit)mascotCustomOutfit.disabled=!1}});setTimeout(()=>{mascotSmartCheck.dispatchEvent(new Event('change'))},200)}
const smartAutoCheckbox=document.getElementById('banana-smart-auto-checkbox');const manualContainer=document.getElementById('manual-config-container');const textCheckbox=document.getElementById('banana-text-overlay-checkbox');const textOverlayBox=textCheckbox?textCheckbox.closest('.special-box'):null;if(smartAutoCheckbox){smartAutoCheckbox.addEventListener('change',(e)=>{const isSmartOn=e.target.checked;if(manualContainer){if(isSmartOn)manualContainer.classList.add('disabled-section');else manualContainer.classList.remove('disabled-section')}
if(textOverlayBox&&textCheckbox){textOverlayBox.classList.remove('disabled-section')}});setTimeout(()=>{smartAutoCheckbox.dispatchEvent(new Event('change'))},100)}
